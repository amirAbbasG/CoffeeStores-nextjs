...
const getFetchCoffeeStoresUrl = (latLong, query, limit) => {
  return `https://api.foursquare.com/v3/places/nearby?ll=${latLong}&
    query=${query}&client_id=${process.env.FOURSQURE_CLIENT_ID}&
    client_secret=${process.env.FOURSQURE_CLIENT_SECRET}&v=20220221&limit=${limit}`;
};
...
export const fetchCoffeeStores = async () => {
  try {
    ...
    if (!process.env.FOURSQURE_API_KEY || !process.env.UNSPLASH_ACCESS_KEY) {
      console.error(
